...
pub fn calc_rate_matrix(
    ...
    match substitution_model {
        ...
        SubstitutionModel::PMSFNoNormalize => {
            calc_rate_matrix_relaxpmsf_nonorm(R, log_pi, global_scaling)
        }
    }
}

/// RelaxPMSF rate matrix. We do not normalize here
pub fn calc_rate_matrix_relaxpmsf_nonorm(
    R: &Tensor,
    log_pi: &Tensor,
    rate: &Tensor,
) -> (Tensor, Tensor) {
    ...
    let (S, sqrt_pi) = iqtree2phylograd_parametrization(&R, &pi).unwrap();
...
pub fn calc_rate_matrix_relaxpmsf(R: &Tensor, log_pi: &Tensor, rate: &Tensor) -> (Tensor, Tensor) {
    ...
    let substiution_rates = substitution_rates(&S, &sqrt_pi);

    let substiution_rates =
        Tensor::from_vec(substiution_rates, &[L], &candle_core::Device::Cpu).unwrap();
...
pub fn calc_rate_matrix_mutsel(Mu: &Tensor, log_pi: &Tensor, rate: &Tensor) -> (Tensor, Tensor) {
    let L = log_pi.dims()[0];

    let equi_mutation = (Mu
    ...
    let fitness = log_pi
        .broadcast_sub(&log_equi_mutation.unsqueeze(0).unwrap())
        .unwrap();
    ...
    let S = sqrt_pi_Q
        .broadcast_div(&pi_sqrt.unsqueeze(1).unwrap())
        .unwrap();
