...
#[cfg(windows)]
unsafe fn os_read(fd: libc::c_int, buf: &mut [u8]) -> isize {
    unsafe { libc::_read(fd, buf.as_mut_ptr() as *mut libc::c_void, buf.len() as libc::c_uint) as isize }
}
...
#[cfg(windows)]
unsafe fn os_write(fd: libc::c_int, buf: &[u8]) -> isize {
    unsafe { libc::_write(fd, buf.as_ptr() as *const libc::c_void, buf.len() as libc::c_uint) as isize }
}
...
fn create_pipe() -> std::io::Result<[libc::c_int; 2]> {
    let mut fds = [0; 2];
    // _pipe uses CRT file descriptors, which matches _dup/_dup2/_read/_write.
    let ret = unsafe { libc::_pipe(fds.as_mut_ptr(), 4096, 0) };
    if ret != 0 {
        Err(std::io::Error::last_os_error())
    } else {
        Ok(fds)
