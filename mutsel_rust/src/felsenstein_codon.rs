//! Codon-dimension (61-state) counterparts of the `FelsensteinOp`/
//! `FelsensteinWithEdgeOp` custom-op wrappers in `felsenstein.rs`. These
//! structs bake the state count into their field types
//! (`Arc<Mutex<FelsensteinTree<N>>>`), so they can't be parametrized over
//! `N` without changing the existing (20-state, amino-acid) file; this is a
//! parallel, additive file instead.
...
pub struct FelsensteinOp {
    felsenstein: Arc<Mutex<phylo_grad::FelsensteinTree<N>>>,
    result_storage: Mutex<Option<phylo_grad::FelsensteinResult<N>>>,
}
pub struct FelsensteinWithEdgeOp {
    felsenstein: Arc<Mutex<phylo_grad::FelsensteinTree<N>>>,
    result_storage: Mutex<Option<phylo_grad::FelsensteinResultWithSingleTree<N>>>,
}
...
fn tensor2nalgebra2d(s: &candle_core::CpuStorage, l: &candle_core::Layout) -> Vec<phylo_grad::nalgebra::SMatrix<f64, N, N>> {
    ...
    match s {
        CpuStorage::F64(data) => {
            ...
            for _ in 0..batch_size {
                let mut mat = phylo_grad::nalgebra::SMatrix::<f64, N, N>::zeros();
                for i in 0..N {
                    for j in 0..N {
...
fn tensor2nalgebra1d(s: &candle_core::CpuStorage, l: &candle_core::Layout) -> Vec<phylo_grad::nalgebra::SVector<f64, N>> {
    let dims = l.shape().dims();
    let stride = l.stride();
    assert!(dims.len() == 2);
    let batch_size = dims[0];
    ...
    match s {
        CpuStorage::F64(data) => {
            let mut offset = l.start_offset();
            for _ in 0..batch_size {
                let mut vec = phylo_grad::nalgebra::SVector::<f64, N>::zeros();
                for i in 0..N {
...
fn nalgebra2tensor1d(vec: &Vec<phylo_grad::nalgebra::SVector<f64, N>>) -> candle_core::Tensor {
    let batch_size = vec.len();
    let mut data = vec![0f64; batch_size * N];
    for (b, v) in vec.iter().enumerate() {
        for i in 0..N {
            data[b * N + i] = v[i];
        }
    }
    candle_core::Tensor::from_vec(data, &candle_core::Shape::from(&[batch_size, N]), &candle_core::Device::Cpu).unwrap()
}

fn nalgebra2tensor2d(mats: &Vec<phylo_grad::nalgebra::SMatrix<f64, N, N>>) -> candle_core::Tensor {
    let batch_size = mats.len();
    let mut data = vec![0f64; batch_size * N * N];
    for (b, m) in mats.iter().enumerate() {
        for i in 0..N {
            for j in 0..N {
                data[b * N * N + i * N + j] = m[(i, j)];
            }
        }
    }
    candle_core::Tensor::from_vec(data, &candle_core::Shape::from(&[batch_size, N, N]), &candle_core::Device::Cpu).unwrap()
}

impl FelsensteinFwdOp {
    pub fn new(felsenstein: Arc<Mutex<phylo_grad::FelsensteinTree<N>>>) -> Self {
        Self { felsenstein }
    }
}
